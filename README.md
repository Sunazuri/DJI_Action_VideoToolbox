...
- DJI ActionシリーズのD-Log M動画へ `.cube` LUTを適用
- `DJI OSMO Action 4 D-Log M to Rec.709 V1.cube` などを使ったRec.709変換
- NVIDIA NVENCを利用したHEVC/H.265エンコード
- 複数動画ファイルの連結
- FFmpegコマンドをGUIで扱いやすくする補助
...
- `DJI_Action_VideoToolbox_v1.0.8.zip`

GitHubが自動生成する `Source code (zip)` / `Source code (tar.gz)` はソースコードのみです。通常利用する場合は、Release Assets内の配布用ZIPを使用してください。
...
v1.0.8では、エンコード完了後にWindowsを自動終了する機能を改善しています。

- エンコード中でも `処理完了後シャットダウン` のON/OFF変更が可能
- エンコード中でも `エラー終了時も強制シャットダウン` のON/OFF変更が可能
- エンコード中でも待機時間と `秒 / 分` の変更が可能
- シャットダウン判定は、エンコード開始時ではなく **完了直後の最新設定** を採用
- キャンセル終了時はシャットダウンしない
- エラー終了時は通常シャットダウンしない
- `エラー終了時も強制シャットダウン` がONの場合のみ、詳細ログ保存後に指定時間経過でシャットダウン
- シャットダウン待機中はキャンセル可能
...
1. Releaseから `DJI_Action_VideoToolbox_v1.0.8.zip` をダウンロードします。
2. ZIPを任意のフォルダへ展開します。
3. `DJI_Action_VideoToolbox_v1.0.8.exe` を起動します。
...
現行版では、主にNVIDIA NVENC環境を想定しています。AMD / Intel GPU向けの専用モードは、今後の拡張候補です。
...
```text
src/
├─ DJI_Action_VideoToolbox_v1.0.8.csproj
├─ MainForm.cs
├─ Program.cs
├─ app.ico
├─ app.manifest
...
Visual Studioで `DJI_Action_VideoToolbox_v1.0.8.csproj` を開き、Release構成でビルドしてください。
